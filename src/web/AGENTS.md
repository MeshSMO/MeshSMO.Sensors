...
8. **Product context** (корневой AGENTS.md): иерархия MeshSMO (проект) → Sensors (сервис) → MeshCore/LoRa (технология); технология не выше бренда. Пользовательский copy — про «что и зачем»; детали реализации (outbox, BFF, prerender, «API того же домена») в UI/SEO не выносить; обещаний вида «без задержек/посредников» не давать. Измеренные значения, агрегаты и прогноз различаются явно; пропуски не заполняются расчётными данными. Пользовательский copy живёт в `src/i18n/locales/ru-RU/translation.ts` — не хардкодить строки в компонентах.
