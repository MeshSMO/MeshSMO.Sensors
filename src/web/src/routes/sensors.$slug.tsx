        ...
        <PageShell>
            ...
            <p className="num mt-3 text-sm text-muted-foreground">
                ...
                <span className="ml-2 font-sans">
                    {registry.location?.precision === "approximate"
            ...
            <Readings
                ...
                poll={registry.pollIntervalSeconds ?? 60}
            />
            ...
            <History
                ...
                hasExplicitParams={Boolean(
                    search.metric ?? search.metrics ?? search.range ?? search.from ?? search.to ?? search.mode,
                )}
            />
                ...
                <div className="flex overflow-hidden rounded-md border border-border">
                    {(
                        ...
                        <button
                            ...
                            className={`px-3 py-1.5 text-xs transition-colors ${mode === key
            ...
            <div className="mt-3 flex flex-wrap gap-2">
                {ranges.map((key) => (
                    <button
                        ...
                        className={`num rounded-md border px-3 py-1.5 text-xs transition-colors ${key === range
...
function ChartSummary({
    ...
    return (
        <p className="num mt-3 text-xs text-muted-foreground">
            {prefix}среднее{" "}
            {formatNumber(avg.reduce((a, b) => a + b, 0) / avg.length, meta.precision)}{" "}
        ...
        >
                            ...
                            {dates?.from ? (
                                dates.to ? (
                                    <span className="truncate">
                                        {format(dates.from, "d MMM yyyy", { locale: ru })} — {format(dates.to, "d MMM yyyy", { locale: ru })}
                                    </span>
                                ) : (
            ...
            <label className="flex min-w-28 flex-col gap-1 text-xs text-muted-foreground">
                ...
                <span className="relative">
                    <Clock className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2" aria-hidden />
                    <Input
                        inputMode="numeric"
                        value={startTime}
    ...
    return (
        <PageShell>
            ...
            <p className="mt-3 text-muted-foreground">
                Такого датчика нет в публичном реестре MeshSMO.
            </p>
            <Link
                to="/sensors"
