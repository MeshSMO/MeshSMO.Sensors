...
### История измерений (реализовано на BFF)
...
Downsampling на бэкенде (resolution=auto): ≤6h → raw; ≤24h → 5m; ≤7d → 15m; ≤31d → 1h;
≤180d → 6h; >180d → 1d. Можно зафиксировать `resolution` явно (raw/5m/15m/1h/6h/1d) —
тогда действует бюджет точек (~5k): например raw ограничен 24 часами диапазона, весь запрос — 400 днями.
Ошибки: 404 (нет/скрыт датчик), 400 `{error:"ValidationError",message}` (кривой metric/from/to/resolution
или слишком большой диапазон). Пустой `points[]` — валидный ответ («данных за период нет»),
график показывает честное пустое состояние, не ошибку приложения.
