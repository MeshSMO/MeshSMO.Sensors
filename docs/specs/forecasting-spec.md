...
Для MVP добавить отдельную .NET class library `MeshSMO.Sensors.Forecasting` на ML.NET 5.0.0. Единица обучения и оценки — строго одна временная серия `(sensor_id, metric_key)`: температура одного датчика не смешивается с температурой другого, а температура не смешивается с давлением. BFF строит прогноз по запросу из фактических значений PostgreSQL, но не записывает ни прогнозные точки, ни модель в БД. Короткоживущий результат хранится только в памяти процесса; одинаковые параллельные запросы объединяются. Rolling backtest сравнивает SSA с простыми устойчивыми моделями и публикует кандидата с наименьшей ошибкой; тип выбранной модели явно возвращается в API и показывается в UI.
...
Вместе с SSA считать три дешёвых baseline:
...
- `seasonal median` — медиана значений того же времени за последние доступные сутки (не более семи); устойчива к единичному пасмурному дню и выбросам.

Все кандидаты оцениваются на одних и тех же backtest-folds. SSA допускается в строгом режиме только при улучшении лучшего baseline не менее чем на `MaximumMase`; иначе публикуется лучший baseline с честным `model.kind`, а не слабая SSA-кривая. Итоговый кандидат выбирается по MAE, затем RMSE. При равенстве предпочтение отдаётся устойчивому суточному профилю, seasonal naive, last value и только затем SSA.
...
### 9.4.1. LenientMode — осознанный отказ от гейтов

Флаг `Forecasting:LenientMode` (по умолчанию `false`, поведение по умолчанию — все гейты включены) отключает политические проверки, оставляя только технические ограничения. Включать осознанно, когда нужно увидеть прогноз на неидеальных данных и метрики в `model`-диагностике готовы смотреть глазами.

Отключаемые гейты (в скобках — обычное поведение):

- устаревание последнего наблюдения (`stale_data`);
- усечение сегмента по длинному внутреннему гэпу и гэп перед концом ряда (`sparse_data`) — вместо этого все гэпы любой длины заливаются: длинные и хвостовые — hold-last, лидирующие — первым наблюдаемым значением (в diagnostics виден реальный `observedCoverage` и число интерполированных точек);
- минимальная история для горизонта (`insufficient_data`);
- coverage ниже `MinimumCoverage` (`sparse_data`);
- quality gate по MASE / maxMae / interval coverage (`low_quality`);
- material physical-bound violation перед ответом (`low_quality`).

Остаются: `Enabled`, наличие хотя бы одного наблюдения, минимум одна полная суточная сезона в тренировочном сегменте бэктеста, finite-выход модели (NaN в JSON не уедет) и выбор кандидата по реальной backtest-ошибке. Значения ответа по-прежнему клампятся к min/max серии, availability — `ready`; честность метрик и тип выбранной модели сохраняются в diagnostics.

### 9.5. Интервалы — не гарантия

Границы следует называть `forecast interval` / `интервал прогноза`, а не гарантированным диапазоном. Для каждого кандидата интервал калибруется квантилями знаковых остатков rolling backtest и затем проверяется по фактическому coverage. Перед ответом центральная оценка и границы клампятся к физическим min/max серии; для `solar_panel_voltage` BFF по умолчанию задаёт `0…5 В`. Если coverage ниже порога, модель не публикуется в строгом режиме.
...
4. Для горизонтов 1/6/12/24 ч сравнить last-value, seasonal-naive, seasonal-median и SSA candidates.
...
5. SSA выбирается только если прошла порог улучшения по MASE и победила остальные модели по backtest-ошибке; иначе публикуется явно обозначенный лучший baseline.
...
- раздельная калибровка forecast intervals по lead time после накопления достаточного числа backtest-folds;
