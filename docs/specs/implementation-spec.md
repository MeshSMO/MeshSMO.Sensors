...
polling:
  ...
  maxAttempts: 2
  enabled: true
  schedule:                     # опционально: интервал опроса по времени суток (gateway-only)
...
`polling.schedule` — опциональное per-sensor расписание интервалов опроса по времени
суток; учитывается только gateway, в БД и публичный API не попадает. Внутри окна
`[from, to)` интервал опроса равен `interval` окна, вне всех окон действует базовый
`polling.interval`. Окна полуоткрытые (начало включено, конец исключён), `to` ≤ `from`
означает переход через полночь; окна не должны пересекаться, `interval` каждого окна
должен быть больше `polling.timeout`. Времена интерпретируются в часовом поясе
`timeZone` (обязателен при наличии расписания; IANA id, напр. `Europe/Moscow`).
Переключение режима подхватывается при следующей постановке сенсора в очередь после
завершённого цикла (переключение «доезжает» максимум за один интервал); правки самого
YAML применяются при рестарте gateway.
...
Для `temperature` и `humidity` на тех же разрешениях применяется Hampel-фильтр:
текущее среднее сравнивается с медианой до 12 соседних точек, масштаб оценивается через MAD.
Минимально значимое отклонение — `3 °C` и `10 п.п.` соответственно; до накопления шести
соседних точек решение не принимается. Backend читает небольшой контекст перед началом
запрошенного диапазона, но возвращает только сам диапазон. Коды — `temperature_outlier` и
`humidity_outlier`. Междатчиковый эталон не используется без метаданных об условиях установки:
сравнение уличных и комнатных датчиков давало бы систематические ложные срабатывания.
...
Дефолтный бюджет — 5000 точек (`MeasurementResolutionPolicy.MaxPoints`). Параметр `maxPoints`
(целое 100..50000) поднимает бюджет конкретного запроса: допустимый диапазон для бакетного
разрешения масштабируется как `bucket × maxPoints` (raw по-прежнему ограничен 24 часами).
Фронт передаёт его через контрол плотности: процент от исходной частоты измерений
(100% ≈ все точки) переводится в явные `resolution` + `maxPoints` по интервалу опроса сенсора.
...
- peer-group anomaly detection after the registry describes installation conditions.
