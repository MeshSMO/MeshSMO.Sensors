...
**Статус:** v0.2 — реализован полностью (§1–7 и §8) и проверен end-to-end на железе 2026-09-06: gateway опрашивает физический датчик через репитер (`POST /api/request` + ANON-логин bootstrap).
**Дата:** 2026-09-06
**Аудитория:** агент, дорабатывающий прошивку репитера (ветка `vbart-meshcoretel`).
**Родительская спека:** [implementation-spec](./implementation-spec.md) (см. §7.1.3 и Risk A).
...
Формат самого payload прошивка не интерпретирует — она лишь транспорт. Семантика (envelope PollRequest/PollResponse) определена в [implementation-spec](./implementation-spec.md) §7.3 и реализуется на стороне датчика и Gateway.
...
## 8. v0.2 — Анонимный логин (ANON_REQ) — реализовано, проверено end-to-end на железе (2026-09-06)
